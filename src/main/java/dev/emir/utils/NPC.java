...
public class NPC {
    ...
    private Object HideTabPacket;
    private Method setLocation;
    private Object ADD_PLAYER;
    private Object REMOVE_PLAYER;
    ...
    public void spawn() {
        ...
        try {
            ...
            HideTabPacket = Constructor.newInstance(REMOVE_PLAYER, getArray(EntityPlayerClass, EntityPlayer));
            ...
            for (Player p : Bukkit.getOnlinePlayers()) {
                sendPackets(p, hide ? HideTabPacket : TabPacket, SpawnPacket);
            }
            addPlayerConnection();
    ...
    public NPC(String name, Location loc, String skin, boolean hide) {
        try {
            this.skin = skin;
            this.name = name;
            this.loc = loc;
    ...
    public NPC(String name, Location loc) {
        this(name, loc, name, true);
    }

    public void show(Player p) {
        sendPackets(p, this.hide ? HideTabPacket : TabPacket, SpawnPacket);
        sendPackets(p, packetOutEntity, packetHead);
    }

    public void hideFromTab(Player p){
        sendPackets(p, HideTabPacket);
        this.hide = true;
    }
    public void showFromTab(Player p){
        sendPackets(p, TabPacket);
        this.hide = false;
    }
